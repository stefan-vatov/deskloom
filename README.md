...
It wraps [`hyprflow`](https://github.com/isorensen/hyprflow) and provides:
...
- a one-click AUR install action when `hyprflow` is missing. The installer opens
...
When both settings are enabled, Deskloom restores the selected preset shortly
after the Omarchy shell starts. The restore is additive, matching the normal
hyprflow restore behavior; use the snapshot's **Replace** action when you want
to close the current windows first.
...
because it was cloned from a website. Deskloom detects `hyprflow` and exposes a
first-run install button. That button opens Omarchy's visible package installer:

```bash
omarchy-launch-floating-terminal-with-presentation omarchy-pkg-aur-add hyprflow
```
